        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('pingshenzhuanjia','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('pingshenzhuanjia','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('pingshenzhuanjia','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('pingshenzhuanjia','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiangmushenbao/upload/pingshenzhuanjiaMuBan.xls"
                        >批量导入评审专家数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('pingshenzhuanjia','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="pingshenzhuanjiaUploadSuccess"
                                :on-error="pingshenzhuanjiaUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('pingshenzhuanjia','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入评审专家数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('pingshenzhuanjia','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "pingshenzhuanjia.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('pingshenzhuanjia','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="pingshenzhuanjiaIdNumber"
                                   header-align="center"
                                   label="评审专家身份证号">
                        <template slot-scope="scope">
                            {{scope.row.pingshenzhuanjiaIdNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="pingshenzhuanjiaEmail"
                                   header-align="center"
                                   label="电子邮箱">
                        <template slot-scope="scope">
                            {{scope.row.pingshenzhuanjiaEmail}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('pingshenzhuanjia','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('pingshenzhuanjia','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('pingshenzhuanjia','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('pingshenzhuanjia','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                username : null,
                password : null,
                pingshenzhuanjiaName : null,
                pingshenzhuanjiaPhone : null,
                pingshenzhuanjiaIdNumber : null,
                pingshenzhuanjiaPhoto : null,
                sexTypes : null,
                pingshenzhuanjiaEmail : null,
                yanjiufangxiangTypes : null,
                pingshenzhuanjiaContent : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'pingshenzhuanjia',//当前表表名,
                        sumColum : 'pingshenzhuanjia_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'pingshenzhuanjia_name',//分组字符串字段
                        // types : 'pingshenzhuanjia_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "pingshenzhuanjia_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['pingshenzhuanjiaDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "pingshenzhuanjia/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "pingshenzhuanjia/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            pingshenzhuanjiaUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "pingshenzhuanjia/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            pingshenzhuanjiaUploadError(data){
